...
public class AdvancedCalculatorHandler implements AdvancedCalculator.Iface {
    ...
    @Override
    public Map<Integer, Integer> squares(List<Integer> numbers) throws TException {
        if(numbers.isEmpty()){
            throw new InvalidArguments(0, "no data");
        }

        var squares = numbers.stream().map(x -> x*x).collect(Collectors.toList());
        return IntStream.range(0, numbers.size()).boxed().collect(Collectors.toMap(numbers::get, squares::get));
    }
